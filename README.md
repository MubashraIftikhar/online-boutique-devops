...
This is a trimmed-down version of Google's Online Boutique for a DevOps pipeline
learning project. Kept services: `frontend`, `productcatalogservice`, `cartservice`
(includes `redis-cart`). All other services, Istio/Kustomize/Terraform tooling,
and Google's CI workflows have been removed to keep the scope manageable while
learning to build the pipeline from scratch.

Note: since `frontend` normally calls all 11 services, browsing products and
adding to cart will work, but pages/actions depending on removed services (ads,
currency conversion, recommendations, checkout, shipping) will error until/unless
those services are added back in a later phase.
